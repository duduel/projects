...
<<<<<<< HEAD
    ...
    assert(pid >= 0);
    if (pid == 0){ 
       const char* arg[this->args.size() + 1];
       for (unsigned int i = 0; i < this->args.size(); i++) {
           arg[i] = this->args[i].c_str();
       }
       arg[this->args.size()] = nullptr;
       int err = execvp(arg[0], (char**) arg);
       if (err == -1) {
           _exit(1);
       }
       _exit(0);
...
=======
    ...
    fprintf(stderr, "command::make_child not done yet\n");
    return this->pid;
>>>>>>> f63d94f9e7d438c062228cfeb0c7194f61f4a130
}
...
void run(command* c) {
    while (c != nullptr && c->args.size() > 0){
        int p = c->make_child(0);
        int status;
        switch (c->link) {
            case TYPE_NORMAL:
                waitpid(p, &status, 0);
