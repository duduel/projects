...
let oat_alloc_struct ct (id:Ast.id) : Ll.ty * operand * stream =
  let size_of_strct = Int64.of_int ((List.length (TypeCtxt.lookup id ct)) * 8) in 
  let ans_id, arr_id = gensym "array", gensym "raw_array" in
  let ans_ty = cmp_ty ct @@ TRef (RStruct id) in
  let arr_ty = Ptr I64 in
  ans_ty, Id ans_id, lift
    [ arr_id, Call(arr_ty, Gid "oat_malloc", [I64, Const size_of_strct])
    ; ans_id, Bitcast(arr_ty, Id arr_id, ans_ty) ]
...
let rec cmp_exp (tc : TypeCtxt.t) (c:Ctxt.t) (exp:Ast.exp node) : Ll.ty * Ll.operand * stream =
  ...
  *)
  | Ast.Length e ->
      let ret_ty, ret_op, strm = cmp_exp tc c e in
        begin match ret_ty with 
          | Ptr (Struct ty_lst) ->
            let gens_geptr = gensym "len_geptr" in 
              let gens_load = gensym "len_load" in 
                I64, Id gens_load, strm >@ 
                                  [I(gens_geptr, Gep(ret_ty, ret_op, [Const 0L; Const 0L]));
                                  I(gens_load, Load(Ptr I64, Id gens_geptr))]
          | _ -> failwith "wrong array struct type"
      end
  | Ast.Index (e, i) ->
  ...
  *)
   ...
   *)
  | Ast.CStruct (id, l) ->
      let arr_typ, arr_op, arr_strm = oat_alloc_struct tc id in
      let fields_strm = List.fold_left (fun strm (field_ind, field_exp) ->
                                        let field_exp_typ, _, _ = cmp_exp tc c (snd (List.nth l field_ind)) in
                                          let elt_op, elt_code = cmp_exp_as tc c field_exp field_exp_typ in
                                            let index = gensym "Cstruct_index" in 
                                            strm >@ elt_code >@ lift
                                              [ index, Gep(arr_typ, arr_op, [Const 0L; i64_op_of_int field_ind ])
                                              ; gensym "store",  Store(field_exp_typ, elt_op, Id index) ] 
                                    ) [] (List.mapi (fun index exp -> index, snd exp) l) in
        arr_typ, arr_op, arr_strm >@ fields_strm  
  | Ast.Proj (e, id) ->
    let ans_ty, ptr_op, code = cmp_exp_lhs tc c exp in
...
and cmp_exp_lhs (tc : TypeCtxt.t) (c:Ctxt.t) (e:exp node) : Ll.ty * Ll.operand * stream =
  ...
  *)
  | Ast.Proj (e, i) ->
    let strm_typ, strm_op, strm = cmp_exp tc c e in 
      let gens_geptr = gensym "proj_geptr" in 
        let field_id =
          begin match strm_typ with 
            | Ptr (Namedt id) -> id
            | _ -> failwith "wrong proj exp type"
          end in 
            let field_typ, field_ind = TypeCtxt.lookup_field_name field_id i tc in 
              cmp_ty tc field_typ, (Id gens_geptr), strm >@ 
                                                [I(gens_geptr, Gep(strm_typ, strm_op, [i64_op_of_int 0; i64_op_of_int (Int64.to_int field_ind)]))]
...
and cmp_stmt (tc : TypeCtxt.t) (c:Ctxt.t) (rt:Ll.ty) (stmt:Ast.stmt node) : Ctxt.t * stream =
  ...
  *)
  | Ast.Cast (typ, id, exp, notnull, null) ->
      let exp_typ, exp_op, exp_strm = cmp_exp tc c exp in
        let ty = Ptr (cmp_rty tc typ) in
          let gens_notnull = gensym "notnull" in
            let gens_null = gensym "null" in
              let merge = gensym "merge" in
                let gens_guard = gensym "cast_guard" in
                  let allocad = gensym "cast_alloca" in
                    let gens_bitcast = gensym "cast_bitcast" in
                      let guard = 
                        [] >:: I (gens_guard, Icmp (Eq, exp_typ, exp_op, Null)) >:: 
                              T (Cbr (Id gens_guard, gens_null, gens_notnull)) in
                        let cast = 
                          [] >:: I (allocad, Alloca ty) >::
                                I (gens_bitcast, Bitcast (exp_typ, exp_op, ty)) >::
                                I (gensym "cast_store", Store (ty, Id gens_bitcast, Id allocad)) in
                          let new_ctxt = Ctxt.add c id (Ptr ty, Id allocad) in
                            let _, notnull_strm = cmp_block tc new_ctxt rt notnull in
                              let _, null_strm = cmp_block tc c rt null in
                                c, exp_strm >@ guard >:: 
                                              L gens_notnull >@ cast >@ notnull_strm >:: 
                                              T (Br merge) >:: 
                                              L gens_null >@ null_strm >::
                                              T (Br merge) >::
                                              L merge
...
let rec cmp_gexp c (tc : TypeCtxt.t) (e:Ast.exp node) : Ll.gdecl * (Ll.gid * Ll.gdecl) list =
  ...
  | CStruct (id, cs) ->
      let c_gdecl, add_gdecls = List.split (List.map (fun field ->
                                              cmp_gexp c tc (List.assoc field.fieldName cs)
                                            ) (TypeCtxt.lookup id tc)
                                  ) in
        let flat_add_gdecls= List.flatten add_gdecls in
          let g_struct = (Namedt id, GStruct c_gdecl) in
            let gid = gensym "global_struct" in
              (Ptr (Namedt id), GGid gid), flat_add_gdecls @ [(gid, g_struct)]
